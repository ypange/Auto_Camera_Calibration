import cv2
import numpy as np


def on_trackbar(s):
    pass

cv2.namedWindow("Frame", cv2.WINDOW_NORMAL)
cv2.createTrackbar("Distortion_Factor", "Frame", 200, 1000, on_trackbar)

img = cv2.imread("/home/jaiama/Auto_Camera_Calibration/snap2.jpg")
xs = list(range(1920)) * 1080
ys = []
for i in range(1080):
    t = [i]
    y = t * 1920
    ys.append(y)
ys = np.ravel(ys)
xs = np.array(xs)
ys = np.array(ys)
rx, ry = xs - 960., ys - 540.
r_ = (rx ** 2 + ry ** 2) ** 0.5
dist = ((1080 ** 2) + (1920 ** 2)) ** 0.5

cv2.imshow("Frame", img)

while 1:
    factor = (cv2.getTrackbarPos("Distortion_Factor", "Frame") + 1) / 100.0
    print(factor)
    r = r_ / (dist / factor)
    theta = np.zeros(1920 * 1080)
    theta[np.where(r == 0)] = 1.0
    indices = np.where(r != 0)
    theta[indices] = np.arctan(r[indices]) / r[indices]
    sxs, sys = rx * theta + 960, ry * theta + 540
    a = np.reshape(sxs, (-1, 1920)).astype(np.float32)
    b = np.reshape(sys, (-1, 1920)).astype(np.float32)
    dst = cv2.remap(img, a, b, cv2.INTER_LINEAR)
    cv2.imshow("Frame", dst)
    if cv2.waitKey(1) == 27:
        break

cv2.waitKey(0)
